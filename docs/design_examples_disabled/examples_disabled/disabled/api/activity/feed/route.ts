...
async function fetchGithubEvents(username: string): Promise<FeedItem[]> {
  ...
  try {
    const response = await fetch(
      `https://api.github.com/users/${encodeURIComponent(username)}/events/public?per_page=4`,
      {
        headers,
...
export async function GET(request: Request) {
  ...
  try {
    const [posts, members] = await Promise.all([
      listPublishedPosts({ page: 1, pageSize: 10 }),
    ...
    const githubItemsRaw = await Promise.all(
      githubUsernames.map((username) => fetchGithubEvents(username)),
    );
    const githubItems = githubItemsRaw.flat();

    const items = [...postItems, ...githubItems]
