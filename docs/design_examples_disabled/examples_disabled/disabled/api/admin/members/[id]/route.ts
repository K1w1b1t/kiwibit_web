...
import {
  ...
  softDeleteDirectoryMember,
  updateDirectoryMember,
} from '@/lib/member-directory-store';
...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  try {
    const checked = await requireAnyRole(request, 'admin-members-update', 80, [
      'admin',
      'member_manager',
    ]);
    ...
    const updated = await updateDirectoryMember(id, {
      ...
      account_email: payload.account_email
        ? sanitizeText(payload.account_email, 320).toLowerCase()
        : undefined,
      account_password: payload.account_password
        ? sanitizeText(payload.account_password, 120)
        : undefined,
...
export async function DELETE(request: Request, { params }: RouteParams) {
  const ctx = createApiRequestContext(request);
  try {
    const checked = await requireAnyRole(request, 'admin-members-delete', 80, [
      'admin',
      'member_manager',
    ]);
    if ('response' in checked) return withRequestId(ctx, checked.response);
    ...
    if (!deleted) {
      return jsonApiError(ctx, 404, 'Member not found');
    }
