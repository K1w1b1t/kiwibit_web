...
import {
  getMemberFunnelMetrics,
  getMemberFunnelSeries,
  seriesToCsv,
} from '@/lib/member-funnel-metrics';

export async function GET(request: Request) {
  const ctx = createApiRequestContext(request);
  try {
    const checked = await requireAnyRole(request, 'admin-members-metrics', 120, [
      'admin',
      'member_manager',
    ]);
