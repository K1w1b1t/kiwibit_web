...
export async function GET(request: Request) {
  ...
  try {
    ...
    return withRequestId(
      ctx,
      NextResponse.json({ posts: posts.filter((post) => post.authorId === session.memberId) }),
    );
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (payload.action === 'preview') {
      ...
      return withRequestId(
        ctx,
        NextResponse.json({ ok: true, previewUrl: `/blog/preview/${payload.slug}?token=${token}` }),
      );
    }
    ...
    if (payload.action === 'submit_review') {
      const post = await transitionPostStatus(
        payload.slug,
        'in_review',
        actorRoleForTransition(session.role),
      );
    ...
    if (payload.action === 'approve') {
      ...
      const post = await transitionPostStatus(
        payload.slug,
        approvers.length >= 2 ? 'published' : 'in_review',
        'admin',
      );
      if (!post) return jsonApiError(ctx, 404, 'Not found');
      revalidateTag('blog-posts');
