...
export async function POST(request: Request) {
  ...
  try {
      ...
      try {
        ...
        if (!scan.ok) {
          return NextResponse.json(
            { error: 'Upload blocked by antivirus policy' },
            { status: 400 },
          );
        }
      } catch {
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Image optimizer unavailable (sharp not installed)' },
        { status: 500 },
      );
    }
    ...
    if (width < MIN_DIMENSION || height < MIN_DIMENSION) {
      return NextResponse.json(
        { error: 'Avatar dimensions must be at least 240x240' },
        { status: 400 },
      );
    }
    ...
    if (ratio < MIN_RATIO || ratio > MAX_RATIO) {
      return NextResponse.json(
        { error: 'Avatar aspect ratio must be between 0.6 and 1.8' },
        { status: 400 },
      );
    }
    ...
    if (isObjectStorageEnabled()) {
      try {
        for (const size of SIZES) {
          ...
          const outputBuffer = await sharpModule(buffer)
            .rotate()
            .resize(size.width, size.width, { fit: 'cover', withoutEnlargement: true })
            .webp({ quality: 82 })
            .toBuffer();
          const outputAvif = await sharpModule(buffer)
            .rotate()
            .resize(size.width, size.width, { fit: 'cover', withoutEnlargement: true })
            .avif({ quality: 62 })
            .toBuffer();
