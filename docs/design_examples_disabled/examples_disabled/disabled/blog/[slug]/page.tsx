          ...
          {toc.length === 0 ? (
            ...
            <ul className="mt-3 space-y-2 text-sm">
              {toc.map((item) => (
                <li
                  key={item.id}
                  className={item.level === 3 ? 'pl-3 text-[var(--text-soft)]' : ''}
                >
          ...
          <header>
            ...
            <div className="mt-4 flex gap-2">
              <TrackedExternalLink
                ...
                className="rounded border border-[var(--surface-border)] px-3 py-1 text-xs"
              >
                Share LinkedIn
              </TrackedExternalLink>
              <TrackedExternalLink
              ...
              if (block.type === 'h2')
                return (
                  <h2
                    key={`${block.id}-${index}`}
                    id={block.id}
                    className="pt-6 text-3xl font-semibold leading-tight text-[var(--text-main)]"
                  >
                    {block.text}
                  </h2>
                );
              if (block.type === 'h3')
                return (
                  <h3
                    key={`${block.id}-${index}`}
                    id={block.id}
                    className="pt-3 text-2xl font-semibold leading-tight text-[var(--text-main)]"
                  >
                    {block.text}
                  </h3>
                );
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              {related.map((item) => (
                <Link
                  key={item.slug}
                  href={`/blog/${item.slug}`}
                  className="rounded border border-[var(--surface-border)] bg-[var(--surface-card)] p-4"
                >
