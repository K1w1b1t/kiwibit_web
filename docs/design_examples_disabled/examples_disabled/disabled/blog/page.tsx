...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'Blog | KIWI BIT',
    description:
      'Security engineering articles, incident response notes and architecture research.',
    type: 'website',
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-10">
          ...
          <p className="mt-3 max-w-2xl text-sm text-[var(--text-soft)]">
            Draft-to-publish workflow, technical analysis, and practical security playbooks.
          </p>
          <div className="mt-3 flex gap-2 text-xs uppercase tracking-[0.12em]">
            <Link href="/blog" className="rounded border border-[var(--surface-border)] px-2 py-1">
              PT-BR
            </Link>
            <Link
              href="/en/blog"
        ...
        <form action="/blog" className="mb-6 grid gap-3 md:grid-cols-[1fr_auto]">
          ...
          <button
            type="submit"
            className="rounded border border-[var(--surface-border)] px-4 py-2 text-xs uppercase tracking-[0.14em]"
          >
            Search
          </button>
        </form>
        ...
        <section className="mb-8 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {PILLAR_TOPICS.map((topic) => (
              <Link
                key={topic}
                href={`/blog/pillars/${topic}`}
                className="rounded border border-[var(--surface-border)] px-3 py-2 text-xs uppercase tracking-[0.12em]"
              >
                {topic}
              </Link>
            ))}
        ...
        {listing.featured ? (
          <article className="mb-10 overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-card)]">
            <div className="grid gap-0 md:grid-cols-2">
              <Image
                src={listing.featured.coverImage}
                alt={listing.featured.title}
                width={900}
                height={500}
              ...
              <div className="p-8">
                ...
                <Link
                  href={`/blog/${listing.featured.slug}`}
                  className="mt-6 inline-block rounded border border-[var(--text-main)] px-4 py-2 text-xs uppercase tracking-[0.14em]"
                >
                  Read article
                </Link>
              </div>
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          ...
          {(params.tag || params.category || params.q) && (
            <Link
              href="/blog"
              className="rounded border border-[var(--surface-border)] px-3 py-1 text-xs text-[var(--text-muted)]"
            >
              Clear filters
            </Link>
          )}
        ...
        {listing.items.length === 0 ? (
          <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-8 text-center">
            ...
            <Link
              href="/blog"
              className="mt-4 inline-block rounded border border-[var(--surface-border)] px-4 py-2 text-xs uppercase tracking-[0.14em]"
            >
              Show all posts
            </Link>
          </div>
            ...
            {listing.items.map((post) => (
              ...
              >
                ...
                <p className="mt-3 text-[11px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
                  ...
                  {estimateReadingMinutes(post.publishedContent ?? post.draftContent)} min read
                </p>
                <h3 className="mt-2 text-xl font-semibold">{post.title}</h3>
                <p className="mt-2 text-sm text-[var(--text-soft)]">{post.excerpt}</p>
                <Link
                  href={`/blog/${post.slug}`}
                  className="mt-4 inline-block text-xs uppercase tracking-[0.14em] text-[var(--text-muted)] hover:text-[var(--text-main)]"
                >
                  View post
                </Link>
              </article>
        ...
        <div className="mt-8 flex items-center justify-between">
          ...
          <Link
            href={pageHref({
              ...
              tag: params.tag,
              category: params.category,
              page: Math.min(listing.totalPages, listing.page + 1),
            })}
            className="text-xs uppercase tracking-[0.14em]"
          >
            Next
