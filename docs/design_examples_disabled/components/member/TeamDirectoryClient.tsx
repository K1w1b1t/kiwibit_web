...
export default function TeamDirectoryClient() {
  ...
  useEffect(() => {
    ...
    async function loadMembers() {
      ...
      const [response, rankingResponse] = await Promise.all([
        fetch('/api/members'),
        fetch('/api/member/reputation/ranking'),
      ]);
      if (!response.ok) {
  ...
  const filtered = useMemo(() => {
    ...
    return members.filter((member) =>
      `${member.name} ${member.role} ${member.specialties.join(' ')}`.toLowerCase().includes(q),
    );
  }, [members, query]);
    ...
    <section className="mx-auto max-w-6xl px-4 py-24 text-[var(--text-main)]">
      <header className="mb-8">
        ...
        <p className="mt-3 max-w-2xl text-sm text-[var(--text-soft)]">
          Dynamic team data loaded from API with profile links and specialties.
        </p>
      </header>
        ...
        <section className="mb-6 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-4">
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-3">
            {ranking.slice(0, 6).map((item, index) => (
              <div
                key={item.memberId}
                className="rounded border border-[var(--surface-border)] p-3 text-xs"
              >
      ...
      {error ? (
        <p className="rounded border border-red-400/40 bg-red-950/20 px-3 py-2 text-sm text-red-200">
          {error}
        </p>
      ) : null}

      {!isLoading && !error && filtered.length === 0 ? (
        <p className="rounded border border-[var(--surface-border)] bg-[var(--surface-card)] px-4 py-3 text-sm text-[var(--text-soft)]">
          No members found.
        </p>
      ) : null}
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {member.specialties.map((item) => (
                  <span
                    key={`${member.id}-${item}`}
                    className="rounded border border-[var(--surface-border)] px-2 py-1 text-[10px] uppercase tracking-[0.14em]"
                  >
                    {item}
                  </span>
                ))}
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs">
                {member.github_url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="rounded border border-[var(--surface-border)] px-2 py-1 hover:border-[var(--text-main)]"
                  >
                    GitHub
                  </a>
                ) : null}
