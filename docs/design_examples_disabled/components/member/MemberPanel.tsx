    ...
    <section id="home" className="bg-[var(--surface-bg)] pb-28 pt-32">
      <div className="mx-auto grid max-w-6xl items-center gap-14 px-4 md:px-8 lg:grid-cols-2">
        <div>
          ...
          <h1 className="text-6xl font-semibold leading-tight text-[var(--text-main)] md:text-7xl">
            Hello
          </h1>
          <p className="mt-6 max-w-md text-[var(--text-soft)]">
            I&apos;m {member.realName}, {member.speciality}. I build clean and elegant digital
            experiences focused on performance and impact.
          </p>
      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-4 md:px-8 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-6 shadow-sm">
          ...
          <p className="mt-4 text-sm text-[var(--text-soft)]">
            "We saw a massive improvement in engagement. The design was not just beautiful but
            effective."
          </p>
        </div>
      </div>
            ...
            <div className="flex items-center gap-4">
              {githubProfile ? (
                <Image
                  ...
                  width={52}
                  height={52}
                  className="h-13 w-13 rounded-full border border-[var(--surface-border)]"
                />
              ) : (
              ...
              <div>
                ...
                <p className="text-sm text-[var(--text-soft)]">
                  {githubProfile
                    ? githubProfile.bio
                    : 'Member repositories synced from GitHub API.'}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
              {githubProfile ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="rounded border border-[var(--surface-border)] px-3 py-1 hover:border-[var(--text-main)] hover:text-[var(--text-main)]"
                >
              ...
              <span className="rounded border border-[var(--surface-border)] px-3 py-1">
                {githubMetrics?.totalStars ?? repos.reduce((sum, repo) => sum + repo.stars, 0)}{' '}
                stars
              </span>
              {reputation ? (
                <span className="rounded border border-[var(--surface-border)] px-3 py-1">
                  rep {reputation.level} ({reputation.score})
                </span>
              ) : null}
              {githubProfile ? (
                <span className="rounded border border-[var(--surface-border)] px-3 py-1">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {repos.map((repo) => (
              <motion.article
                key={repo.htmlUrl}
                whileHover={{ y: -4 }}
                className="group rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-5"
              >
    ...
    <section id="projects" className="pb-24">
      <div className="mx-auto max-w-6xl px-4 md:px-8">
        <div className="mb-6 rounded-xl border border-dashed border-[var(--surface-border)] bg-[var(--surface-card)] p-4 text-sm text-[var(--text-soft)]">
          GitHub profile not found for this member yet. Configure username in
          `data/member-github.ts`.
        </div>
      </div>
...
function ContactForm({ member, onCtaClick }: { member: Member; onCtaClick?: () => void }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      ...
      <p
        aria-live="polite"
        className={`text-sm ${status === 'error' ? 'text-red-300' : 'text-[var(--text-soft)]'}`}
      >
        {feedback}
      </p>
    </form>
...
function MemberIntelSection({ member, onCtaClick }: { member: Member; onCtaClick?: () => void }) {
  ...
  return (
    <section
      id="reports"
      className="relative mt-24 overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-bg)] text-[var(--text-main)]"
    >
      ...
      <div className="relative px-8 py-20 md:px-16">
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-3">
              {technicalSkills.slice(0, 4).map((skill) => (
                <li
                  key={skill.name}
                  className="border border-[var(--surface-border)] bg-[var(--surface-card)] px-4 py-2 text-sm backdrop-blur-sm"
                >
                  {skill.name}
                </li>
              ))}
        ...
        <div className="mt-14 grid gap-10 md:grid-cols-2">
          <div>
            <h3 className="mb-4 text-sm uppercase tracking-[0.3em] text-[var(--text-muted)]">
              Toolset / Stack
            </h3>
            <div className="flex flex-wrap gap-2">
              {member.stack.map((tool) => (
                <span
                  key={tool}
                  className="rounded border border-[var(--surface-border)] bg-[var(--surface-card)] px-3 py-1.5 text-xs uppercase tracking-[0.14em]"
                >
                  {tool}
                </span>
              ))}
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-[1fr_auto] md:items-end">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            or email directly: {member.contactEmail}
          </p>
        </div>
      </div>
    </section>
...
export default function MemberPanel({ initialMemberId, initialMemberData }: MemberPanelProps) {
  ...
  const safeMemberId =
    initialMemberId && MEMBERS_BY_ID[initialMemberId] ? initialMemberId : defaultMemberId;
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-bg)] font-sans text-[var(--text-main)]">
      ...
      <div aria-live="polite" className="sr-only">
        {isSwitchingMember
          ? 'Loading selected member profile'
          : `Viewing profile for ${member.realName}`}
      </div>
      <Header
        selectedId={safeMemberId}
