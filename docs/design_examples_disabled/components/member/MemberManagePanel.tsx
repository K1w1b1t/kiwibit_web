...
function projectsToText(projects: EditableProject[]) {
  return projects
    .map((project) => `${project.title} | ${project.image} | ${project.href}`)
    .join('\n');
}
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-header)] p-5 backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                ...
                className="rounded border border-[var(--surface-border)] px-3 py-2 text-xs uppercase tracking-[0.14em] text-[var(--text-muted)]"
              >
                View Public
              </Link>
              {role === 'admin' ? (
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.16em]">
            <span
              className={`rounded border px-2 py-1 ${dirty ? 'border-amber-300/60 text-amber-200' : 'border-emerald-300/60 text-emerald-200'}`}
            >
              {dirty ? 'Unsaved edits' : 'No local edits'}
            </span>
            <span className="rounded border border-sky-300/60 px-2 py-1 text-sky-200">
              {moderationStatus}
            </span>
            {savedAt ? <span className="text-[var(--text-muted)]">Saved at {savedAt}</span> : null}
            {pendingAt ? (
              <span className="text-[var(--text-muted)]">
                Pending since {formatDate(pendingAt)}
              </span>
            ) : null}
          </div>
          {previewUrl ? (
            <p className="mt-3 text-xs text-[var(--text-soft)]">
              Preview:{' '}
              <Link className="underline" href={previewUrl}>
                {previewUrl}
              </Link>
            </p>
          ) : null}
        ...
        <div className="grid gap-6 xl:grid-cols-[320px_1fr_320px]">
          <aside className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-5">
            <div className="mx-auto w-fit overflow-hidden rounded-2xl border border-[var(--surface-border)]">
              <Image
                ...
                width={260}
                height={300}
                className="h-[300px] w-[260px] object-cover"
              />
            </div>
            <div className="mt-5 space-y-3">
              ...
              {rawPreviewImage ? (
                <div className="space-y-3 rounded border border-[var(--surface-border)] p-3">
                  <div
                    className="h-40 w-full rounded border border-[var(--surface-border)] bg-center bg-no-repeat"
                    style={{
                      backgroundImage: `url(${rawPreviewImage})`,
                  ...
                  <button
                    ...
                    className="rounded border border-[var(--text-main)] px-3 py-2 text-[10px] uppercase"
                  >
              ...
              {uploadFeedback ? (
                <p className="text-xs text-[var(--text-soft)]">{uploadFeedback}</p>
              ) : null}
            </div>
          </aside>
          ...
          <section className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-6">
            <form onSubmit={handleSaveDraft} className="space-y-6">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => markDirty({ ...member, realName: e.target.value })}
                    className="w-full rounded border border-[var(--surface-border)] bg-black/40 px-3 py-2 text-sm"
                  />
                </label>
                <label className="space-y-2">
                  <span className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
                  ...
                  <input
                    ...
                    onChange={(e) => markDirty({ ...member, speciality: e.target.value })}
                    className="w-full rounded border border-[var(--surface-border)] bg-black/40 px-3 py-2 text-sm"
                  />
                </label>
                <label className="space-y-2">
                  ...
                  <input
                    disabled={role !== 'admin'}
                    value={member.clearance}
                    onChange={(e) => markDirty({ ...member, clearance: e.target.value })}
                    className="w-full rounded border border-[var(--surface-border)] bg-black/40 px-3 py-2 text-sm disabled:opacity-50"
                  />
                </label>
                <label className="space-y-2">
                  <span className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
                  ...
                  <input
                    ...
                    onChange={(e) => markDirty({ ...member, contactEmail: e.target.value })}
                    className="w-full rounded border border-[var(--surface-border)] bg-black/40 px-3 py-2 text-sm disabled:opacity-50"
                  />
                </label>
              </div>
              ...
              <label className="space-y-2 block">
                ...
                <textarea
                  value={member.bio}
                  onChange={(e) => markDirty({ ...member, bio: e.target.value })}
                  rows={4}
                  className="w-full rounded border border-[var(--surface-border)] bg-black/40 px-3 py-2 text-sm"
                />
              </label>
              ...
              <label className="space-y-2 block">
                <span className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
                  Projects (title | image | href)
                </span>
                <textarea
                  value={projectsToText(member.projects)}
                  onChange={(e) =>
                    markDirty({ ...member, projects: textToProjects(e.target.value) })
                  }
                  rows={5}
                  className="w-full rounded border border-[var(--surface-border)] bg-black/40 px-3 py-2 text-sm"
                />
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  type="submit"
                  disabled={isSaving}
                  className="rounded border border-[var(--text-main)] px-6 py-3 text-xs uppercase tracking-[0.18em] text-[var(--text-main)] disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="rounded border border-emerald-300/70 px-6 py-3 text-xs uppercase tracking-[0.18em] text-emerald-200 disabled:opacity-50"
                >
                ...
                {role === 'admin' && moderationStatus === 'pending_review' ? (
                  <button
                    ...
                    className="rounded border border-sky-300/70 px-6 py-3 text-xs uppercase tracking-[0.18em] text-sky-200"
                  >
                    Approve Pending
                  </button>
                ) : null}
          ...
          <aside className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-5">
            <h3 className="text-sm uppercase tracking-[0.2em] text-[var(--text-muted)]">
              Version History
            </h3>
            <ul className="mt-4 space-y-3">
              {versions.slice(0, 12).map((version) => (
                <li
                  key={version.id}
                  className="rounded border border-[var(--surface-border)] bg-black/30 p-3"
                >
                  <p className="text-xs text-[var(--text-soft)]">{formatDate(version.createdAt)}</p>
                  <button
                    type="button"
                    onClick={() => handleRevert(version.id)}
                    className="mt-2 rounded border border-[var(--surface-border)] px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-[var(--text-muted)]"
                  >
                    Revert
                  </button>
                </li>
              ...
              <div className="mt-5">
                ...
                <ul className="mt-2 space-y-2">
                  {pendingList.map((item) => (
                    <li
                      key={item.memberId}
                      className="rounded border border-[var(--surface-border)] p-2 text-xs"
                    >
                      <p>{item.memberId}</p>
                      <p className="text-[var(--text-soft)]">
                        {item.pendingAt ? formatDate(item.pendingAt) : ''}
                      </p>
                      <button
                        type="button"
                        onClick={() => handleApprovePendingMember(item.memberId)}
                        className="mt-1 rounded border border-[var(--surface-border)] px-2 py-1 text-[10px] uppercase"
                      >
                        Approve
                      </button>
                    </li>
