...
function validateMemberForm(form: MemberRecord, isCreateMode: boolean) {
  ...
  if (form.account_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.account_email))
    return 'Email da conta inválido.';
  if (isCreateMode && form.account_password && form.account_password.length < 8)
    return 'Senha da conta deve ter pelo menos 8 caracteres.';
  return null;
}
...
export default function MemberAdminPanel() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [membersResponse, metricsResponse] = await Promise.all([
      fetch('/api/admin/members'),
      fetch(`/api/admin/members/metrics?period=${period}`),
    ]);
        ...
        <header className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[var(--text-soft)]">
              Create, edit, soft-delete, and manage account access for team records.
            </p>
          </div>
          <button
            type="button"
          ...
          <div className="mb-6">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <select
                ...
                className="rounded border border-[var(--surface-border)] bg-black/30 px-2 py-1 text-xs"
              >
              ...
              <button
                ...
                className="rounded border border-[var(--surface-border)] px-2 py-1 text-xs uppercase tracking-[0.12em]"
              >
                Export CSV
              </button>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <div className="rounded border border-[var(--surface-border)] bg-[var(--surface-card)] p-3 text-xs">
              ...
              <div className="mt-3 rounded border border-[var(--surface-border)] bg-[var(--surface-card)] p-3 text-[11px] text-[var(--text-soft)]">
                ...
                <div className="grid gap-1">
                  {series.slice(-7).map((point) => (
                    <p key={point.date}>
                      {point.date}: c{point.created} u{point.updated} d{point.deleted} v
                      {point.validationErrors}
                    </p>
                  ))}
                </div>
              </div>
          ...
          <aside className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-4">
            ...
            <div className="mt-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => void runBulkAction('activate', selectedIds)}
                  className="rounded border border-[var(--surface-border)] px-2 py-1 text-[10px] uppercase tracking-[0.12em]"
                >
                  Bulk Activate
                </button>
                <button
                  type="button"
                  onClick={() => void runBulkAction('deactivate', selectedIds)}
                  className="rounded border border-[var(--surface-border)] px-2 py-1 text-[10px] uppercase tracking-[0.12em]"
                >
                  Bulk Deactivate
                </button>
            ...
            {!isLoading ? (
              <form onSubmit={submitForm} className="space-y-3">
                <div className="rounded border border-[var(--surface-border)] p-3 text-xs">
                  ...
                  {!isCreateMode ? (
                    <button
                      type="button"
                      onClick={() =>
                        void runBulkAction(form.is_active ? 'deactivate' : 'activate', [form.id])
                      }
                ...
                <input
                  ...
                  placeholder="Name"
                  className="w-full rounded border border-[var(--surface-border)] bg-black/30 px-3 py-2 text-sm"
                />
                <input
                  value={form.role}
                ...
                <p className="pt-2 text-xs uppercase tracking-[0.12em] text-[var(--text-muted)]">
                  Access Account
                </p>
                <input
                  value={form.account_email ?? ''}
                  onChange={(event) => setForm({ ...form, account_email: event.target.value })}
                ...
                <input
                  ...
                  onChange={(event) => setForm({ ...form, account_password: event.target.value })}
                  placeholder="Account password (optional)"
                  className="w-full rounded border border-[var(--surface-border)] bg-black/30 px-3 py-2 text-sm"
                />
                <select
                  value={form.access_role ?? 'member'}
                  onChange={(event) =>
                    setForm({ ...form, access_role: event.target.value as AccessRole })
                  }
                  className="w-full rounded border border-[var(--surface-border)] bg-black/30 px-3 py-2 text-sm"
                >
                  <option value="member">member</option>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    type="submit"
                    className="rounded border border-[var(--text-main)] px-4 py-2 text-xs uppercase tracking-[0.14em]"
                  >
                  ...
                  {!isCreateMode ? (
                    <button
                      ...
                      className="rounded border border-red-400/60 px-4 py-2 text-xs uppercase tracking-[0.14em] text-red-300"
                    >
                      Soft Delete
                    </button>
                  ) : null}
