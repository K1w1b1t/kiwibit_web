...
export default function BlogAdminPanel() {
  ...
  const hasEditorialTemplate = useMemo(() => {
    ...
    return ['## Intro', '## Problem', '## Solution', '## Conclusion', '## CTA'].every((section) =>
      form.draftContent.includes(section),
    );
  }, [form]);
  ...
  const loadSessionAndData = useCallback(async () => {
    ...
    const [growthResponse, seoGapsResponse] = await Promise.all([
      fetch('/api/admin/blog/growth'),
      fetch('/api/admin/blog/seo-gaps'),
    ]);
    if (!postResponse.ok) {
  ...
  async function runAction(
    action: 'submit_review' | 'publish' | 'schedule' | 'approve' | 'preview',
  ) {
    if (!form?.slug) {
    ...
    if (typeof data.approvals === 'number' && typeof data.requiredApprovals === 'number') {
      setFeedback(
        `Action ${action} completed. Approvals: ${data.approvals}/${data.requiredApprovals}.`,
      );
    } else {
  ...
  const panelClass =
    'rounded-2xl border border-white/15 bg-[rgba(20,20,20,0.6)] backdrop-blur-xl shadow-[0_0_15px_rgba(255,255,255,0.05),inset_0_0_10px_rgba(255,255,255,0.02)]';
  const inputClass =
    'w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white placeholder-zinc-600 focus:border-white/40 focus:outline-none';
          ...
          <section className={`xl:col-span-6 ${panelClass} p-6`}>
            <form onSubmit={savePost} className="space-y-3">
                        ...
                        onClick={() =>
                          setForm({
                            ...
                            categories: selected
                              ...
                              : [...form.categories, category].slice(0, 3),
                          })
                        }
              ...
              <textarea
                ...
                onChange={(e) => setForm({ ...form, draftContent: e.target.value })}
                rows={16}
                className={`${inputClass} min-h-[360px] font-mono`}
              />
              {!hasEditorialTemplate ? (
                <p className="text-xs text-amber-300">
                  Recommendation: include Intro, Problem, Solution, Conclusion and CTA sections.
                </p>
              ) : null}
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  type="submit"
                  className="rounded-lg border border-white/30 bg-white/10 px-4 py-2 text-xs uppercase tracking-[0.14em] text-white"
                >
                  Save
                </button>
                <button
                  type="button"
                  onClick={() => setForm({ ...form, draftContent: BLOG_ARTICLE_TEMPLATE })}
                  className="rounded-lg border border-white/20 px-4 py-2 text-xs uppercase tracking-[0.14em] text-zinc-300"
                >
                  Apply Template
                </button>
                <button
                  ...
                  className="rounded-lg border border-white/70 bg-white px-4 py-2 text-xs uppercase tracking-[0.14em] text-black shadow-[0_0_25px_rgba(255,255,255,0.2)]"
                >
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="rounded-lg border border-white/20 bg-black/40 px-3 py-2 text-xs"
                />
                <button
                  type="button"
              ...
              {previewUrl ? (
                <p className="text-xs text-[var(--text-soft)]">Preview: {previewUrl}</p>
              ) : null}
              ...
              {assist ? (
                <div className="rounded border border-[var(--surface-border)] p-3 text-xs text-[var(--text-soft)]">
                  ...
                  <p className="mt-2">
                    SEO score: <strong>{assist.seoScore}</strong>/100
                  </p>
                  ...
                  <p className="mt-2 text-[10px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
                    Checklist
                  </p>
                  <p className="mt-1">
                    Intro {assist.checklist.hasIntro ? 'OK' : 'Missing'} | Problem{' '}
                    {assist.checklist.hasProblem ? 'OK' : 'Missing'} | Solution{' '}
                    {assist.checklist.hasSolution ? 'OK' : 'Missing'} | Conclusion{' '}
                    {assist.checklist.hasConclusion ? 'OK' : 'Missing'} | CTA{' '}
                    {assist.checklist.hasCTA ? 'OK' : 'Missing'}
                  </p>
          ...
          <section className="space-y-6 xl:col-span-3">
            ...
            <div className={`${panelClass} p-4`}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => void enqueueJob('embeddings_reindex')}
                  className="rounded border border-[var(--surface-border)] px-3 py-2 text-[10px] uppercase tracking-[0.12em]"
                >
                  Queue Embeddings Reindex
                </button>
                <button
                  type="button"
                  onClick={() => void enqueueJob('growth_rollup')}
                  className="rounded border border-[var(--surface-border)] px-3 py-2 text-[10px] uppercase tracking-[0.12em]"
                >
                  Queue Growth Rollup
                </button>
                <button
                  type="button"
                  onClick={() => void enqueueJob('github_sync')}
                  className="rounded border border-[var(--surface-border)] px-3 py-2 text-[10px] uppercase tracking-[0.12em]"
                >
                  Queue GitHub Sync
                </button>
              ...
              {opsFeedback ? (
                <p className="mt-2 text-xs text-[var(--text-soft)]">{opsFeedback}</p>
              ) : null}
            </div>
            <div className={`${panelClass} p-4`}>
              <p className="text-xs uppercase tracking-[0.14em] text-[var(--text-muted)]">
              ...
              <div className="mt-4 space-y-3 text-sm text-[var(--text-soft)]">
                {parsedPreview.slice(0, 16).map((block, index) => {
                  ...
                  if (block.type === 'ul')
                    return (
                      <ul key={`ul-${index}`} className="list-inside list-disc">
                  ...
                  >
                    ...
                    <div className="mt-2 flex gap-2">
                      <button
                        ...
                        className="rounded border border-[var(--surface-border)] px-2 py-1 text-[10px] uppercase"
                      >
                        Approve
                      </button>
                      <button
            ...
            <div className={`${panelClass} p-4`}>
              ...
              <div className="mt-3 overflow-x-auto">
                ...
                {opsRows.length === 0 ? (
                  <p className="px-2 py-3 text-xs text-[var(--text-muted)]">No API metrics yet.</p>
                ) : null}
              </div>
            </div>
            ...
            {growth ? (
              <div className={`${panelClass} p-4`}>
                ...
                <p className="mt-2 text-xs text-[var(--text-soft)]">
                  Retention: {growth.retentionRatePercent}%
                </p>
            ...
            {seoGaps ? (
              <div className={`${panelClass} p-4`}>
                ...
                <p className="mt-2 text-xs text-[var(--text-soft)]">
                  Thin: {seoGaps.thinContentCount} | Missing template:{' '}
                  {seoGaps.missingTemplateCount} | No internal links: {seoGaps.noInternalLinkCount}
                </p>
                <div className="mt-3 grid gap-2 text-xs">
                  {seoGaps.rows.slice(0, 8).map((row) => (
                    ...
                    >
                      ...
                      {row.gaps.length > 0 ? (
                        <p className="mt-1 text-[11px] text-amber-300">{row.gaps.join(' | ')}</p>
                      ) : (
                        <p className="mt-1 text-[11px] text-emerald-300">No critical gaps.</p>
                      )}
                    </div>
                  ))}
                </div>
                <div className="mt-3 rounded border border-[var(--surface-border)] p-3 text-xs">
                  ...
                  <p className="mt-1">
                    {seoGaps.coverage
                      .slice(0, 8)
                      .map((item) => `${item.topic}: ${item.totalPosts}`)
                      .join(' | ') || 'No coverage data.'}
                  </p>
                </div>
              </div>
            ) : null}
