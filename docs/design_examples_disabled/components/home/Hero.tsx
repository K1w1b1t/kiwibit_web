    ...
    function draw() {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          context.fillStyle = isWhite ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.9)';
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden architectural-grid bg-black">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-10">
            <button className="w-full sm:w-auto bg-white text-black px-14 py-7 text-[11px] font-black uppercase tracking-ultra hover:bg-zinc-200 transition-colors">
              Explore Services
            </button>

            <button className="w-full sm:w-auto border border-white/10 px-14 py-7 text-[11px] font-bold uppercase tracking-ultra hover:border-white/40 transition-all text-white/50 hover:text-white">
              View Portfolio
            </button>
          </div>
        </div>
