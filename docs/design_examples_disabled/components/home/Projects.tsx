    ...
    async function loadPostsAndNews() {
      try {
        ...
        const mappedPosts: HomeProjectCard[] = posts.map((post) => ({
          ...
          title: post.title,
          desc: post.excerpt,
          badges: [...(post.tags ?? []).slice(0, 2), 'NEWS'].map((item) =>
        ...
        <div className="max-w-[1600px] mx-auto">
          <div className="flex flex-col md:flex-row items-end justify-between mb-32">
            <div className="flex flex-col">
              <span className="text-[9px] font-mono text-white/30 uppercase tracking-[0.4em] mb-4">
                Section_02 // POSTS_AND_NEWS
              </span>
              ...
              {allProjects.map((proj, idx) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-16">
                    <span className="bg-zinc-100 text-black text-[9px] font-black px-3 py-1 uppercase tracking-widest rounded-sm border border-black/10">
                      {proj.tag}
                    </span>
                    <span className="text-black/60 text-[10px] font-mono border border-black/15 px-2 py-1">
                      {proj.date}
                    </span>
                  </div>
                  <div className="space-y-4 mb-12 flex-grow">
                    ...
                    <p className="text-black/70 font-light text-sm leading-relaxed">{proj.desc}</p>
                  </div>
                  <div className="flex flex-wrap gap-3 mt-auto pt-8 border-t border-black/10">
                    {proj.badges.map((badge, bIdx) => (
                      <span
                        key={bIdx}
                        className="text-[8px] text-black/60 font-bold uppercase tracking-widest"
                      >
                        {badge}
                      </span>
                    ))}
