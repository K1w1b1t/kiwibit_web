    ...
    <section className="matrix-grid-texture relative px-8 py-40" id="team">
      <div className="mx-auto max-w-[1400px]">
        <div className="mb-32 flex flex-col items-center text-center">
          ...
          <h2 className="mb-6 text-5xl font-black uppercase tracking-[0.2em] text-white md:text-7xl">
            OPERATIONAL_MATRIX_V2.0
          </h2>
        ...
        <div className="mb-24">
          <div className="grid grid-cols-1 justify-items-center gap-16 sm:grid-cols-2 md:gap-24 lg:grid-cols-3">
            {TEAM_MEMBER_CARDS.map((member) => (
              <Link
                key={member.slug}
                href={`/member/${member.slug}#projects`}
                className="group relative flex w-full max-w-[280px] flex-col items-center text-center"
              >
                ...
                <div className="space-y-2">
                  ...
                  <span className="block text-[9px] font-mono tracking-widest text-white/60">
                    {member.role}
                  </span>
                  <p className="mt-4 text-[11px] font-mono text-white/40 transition-all duration-500 group-hover:text-white/80">
                    {member.desc}
                  </p>
                  <div className="mt-4 flex items-center justify-center gap-2">
                    <span className="rounded border border-white/20 px-2 py-1 text-[9px] font-mono uppercase tracking-[0.2em] text-white/60">
                      {MEMBER_GITHUB_USERNAMES[member.slug]
                        ? `@${MEMBER_GITHUB_USERNAMES[member.slug]}`
                        : 'GITHUB_PENDING'}
                    </span>
