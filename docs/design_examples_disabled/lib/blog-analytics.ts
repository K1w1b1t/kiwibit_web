...
const TRACKED_ACTIONS: ActionName[] = [
  'scroll_depth',
  'post_dwell',
  'post_cta_click',
  'tag_click',
  ...
  const result: BlogPostAnalytics[] = filteredPosts.map((post) => {
    ...
    const scrollAvgDepth =
      data.scrollDepths.length > 0
        ? data.scrollDepths.reduce((acc, value) => acc + value, 0) / data.scrollDepths.length
        : 0;
    const dwellAvgSec =
      data.dwellMs.length > 0
        ? data.dwellMs.reduce((acc, value) => acc + value, 0) / data.dwellMs.length / 1000
        : 0;
    const engagementBase =
      ...
      data.shareClicks;
    const ctrPercent =
      engagementBase > 0 ? ((data.ctaClicks + data.shareClicks) / engagementBase) * 100 : 0;
