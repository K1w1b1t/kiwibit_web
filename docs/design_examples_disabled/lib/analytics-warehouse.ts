...
export async function rollupAnalyticsDaily(days = 14) {
  ...
  for (let i = 0; i < days; i += 1) {
    ...
    const newsletterStarts = events.filter(
      (e) => e.action === 'newsletter_subscribe_pending',
    ).length;
    const newsletterConfirms = events.filter(
      (e) => e.action === 'newsletter_subscribe_confirmed',
    ).length;
