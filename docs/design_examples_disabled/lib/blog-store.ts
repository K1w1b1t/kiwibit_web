...
function normalizeCategories(categories: string[]) {
  ...
  const normalized = categories.filter(
    (category): category is (typeof BLOG_FIXED_CATEGORIES)[number] =>
      allowed.has(category as (typeof BLOG_FIXED_CATEGORIES)[number]),
  );
...
function applyFilters(posts: BlogPost[], query: BlogQuery) {
  ...
  return posts.filter((post) => {
    ...
    if (q) {
      const text =
        `${post.title} ${post.excerpt} ${post.tags.join(' ')} ${post.categories.join(' ')}`.toLowerCase();
...
export async function upsertDraftPost(input: UpsertPostInput) {
  const normalizedTags = Array.from(new Set(input.tags.map(normalizeTag).filter(Boolean))).slice(
    0,
    12,
  );
  const normalizedCategories = normalizeCategories(
    Array.from(new Set(input.categories)).slice(0, 3),
  );

  if (isDatabaseEnabled()) {
    const computedSlug = normalizeSlug(
      input.slug && input.slug.length > 0 ? input.slug : input.title,
    );
  ...
  if (isDatabaseEnabled()) {
    ...
    const scored = published
      .map((post) => {
        ...
        const categoryScore =
          post.categories.filter((cat) => base.categories.includes(cat)).length * 3;
